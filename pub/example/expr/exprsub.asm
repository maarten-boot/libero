;=============================================================================
;
;  exprsub.asm   Expression calculator 'calc_expr ("1 + 2")'
;
...
;                program.
;
;  Skeleton generated by LIBERO 2.10 on 24 May, 1995, 13:24.
;=============================================================================

;  The LINKAGE macro defines the interface between your function and calling
...
;  starting at <6[bp]>.
;
function         equ     <_calc_expr>  ; Name of your function
FUNCTION_OPEN    macro
...
expr_lo          equ     <6[bp]>       ; Define parameters here;
expr_hi          equ     <8[bp]>       ;    this is a 32-bit address
                 endm

FUNCTION_CLOSE   macro
                 mov     ax,result     ; Get return code
...
include exprsub.d                      ; Include dialog definitions
...
operator_ptr     dw      ?             ; Current size of operator stack
...
operand_ptr      dw      ?             ; Current size of operand stack
operand_stack    dw      operand_max dup (0)

arg_addr         label   dword
arg_ptr          dw      ?             ; Segment offset into expression
arg_seg          dw      ?             ; Segment address of expression

result           dw      ?             ; Calculated result of expression
the_pty          db      ?             ; Current priority
the_token        db      ?             ; Current token
the_number       dw      ?             ; Current number
...
MODULE initialise_the_program
                 les     di,expr_lo    ; Get address of expression
                 mov     arg_seg,es    ;
                 mov     arg_ptr,di    ; Move to start of expression

                 mov     result,0      ; Assume result is zero
                 mov     operator_ptr,0; Clear stacks
                 mov     operand_ptr,0
...
MODULE get_next_token
                 les     di,arg_addr   ; Point ES:DI to expression
get_next_loop:   mov     al,es:[di]    ; Get next byte
                 inc     di            ;   and bump DI
                 cmp     al,' '        ; Skip leading spaces
                 je      get_next_loop ;
                 mov     the_token,al  ; Store al in token
                 mov     arg_ptr,di    ;  and update argument pointer
...
unstack_4:       cmp     dl,end_mark_token
                 ...
                 mov     result,ax     ;   result of expression = AX
                 call    echonum       ;
                 lea     si,newline    ;
                 call    echo          ;
