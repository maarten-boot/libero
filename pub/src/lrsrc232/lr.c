...
local process_file        (char *filename);
local handle_signal       (int Signal);
local ignore_signal       (int Signal);
local print_help          (void);
    ...
    for (argn = 1; argn < argc; argn++)
      {
        if (argv [argn][0] == '-')
          {
            /*  -initial, -help, -path, -dpath processed at once             */
            if (ARGN_IS ('i', 'n', 'i')
            ||  ARGN_IS ('h', 'e', 'l')
            ||  ARGN_IS ('p', 'a', 't')
            ||  ARGN_IS ('d', 'p', 'a'))
