...
{
    /*  Set list head pointers to null */
    listhead->next = listhead->child = last_state = NULL;  /*  Set other pointers to null */
...
)
{
    ...
    if (streq(token, "terminate") || streq(token, "the_next") || streq(token, "the_exception")) {
    ...
    *scanev;
    ...
    if (state->child == event) {                           /*  If first event, update state */
        ...
        for (scanev = state->child; scanev; scanev = scanev->next) {
            if (scanev->next == event) {                   /*  Link previous event to next, if there is any previous */
                scanev->next = next;
                break;
            }
        }
