...
)
{
    ...
    if (!isalpha(*name) || !isalnum(strlast(name))) {
...
)
{
    ...
    for (fmtlen = 0; *name; name++) {
        ...
        if (ch == '-' || ch == '_') {
            ...
            if (title) {
                /*  Capital only at start of name */
                capital = (fmtlen == 0);
            } else if (headline || caps || normal) {
                capital = TRUE;
            }

            if (title || headline || plain) {
                formatted[fmtlen++] = '_';
            } else if (normal || cobol) {
                formatted[fmtlen++] = '-';
            }
        } else {
            if (cobol) {
    ...
    *next;                                                 /*  Pointer to next item in list */
    ...
    for (event = state->child; event;) {
        ...
        for (module = event->child; module;) {
            ...
            PutSymNumber(module->name, (dbyte) (GetSymNumber(module->name) - 1));
            next = module->next;
            free(module);
            module = next;
        }
    ...
    next = state->next;
    if (listhead->child == state) {
        /*  If first state, update listhead */
        listhead->child = next;                            /*    to point to sibling */
    } else {
        for (scanst = listhead->child; scanst; scanst = scanst->next) {
            /*  Link previous state to next */
            if (scanst->next == state) {                   /*    if there is any previous */
                scanst->next = next;
                break;
            }
        }
