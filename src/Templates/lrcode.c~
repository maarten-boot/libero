...
typedef struct DO_BLOCK_tag {
    ...
    function handler;                                      /*  Function that handles action */
...
static DO_BLOCK do_block;                                  /*  Current do_block */

static int
 do_block_level;                                           /*  Current DO level; 0 = none */

local handler_for_do_action(
    void
);
local handler_for_do_event(
    void
);
local handler_for_do_module(
    void
);
local handler_for_do_nextst(
    void
);
local handler_for_do_state(
    void
);
local handler_for_do_vector(
    void
);
local handler_for_do_overflow(
    void
);
local handler_for_do_while(
    void
);
local handler_for_do_stubs(
    void
);
...
static IF_BLOCK if_block;                                  /*  Current if_block */

static int if_block_level;                                 /*  Current IF level; 0 = none */
...
static int feedback,                                       /*  Feedback to calling program */
...
static size_t substr_from,                                 /*  Substring specification */
 substr_size;                                              /*    from :substr action */

static dbyte line_nbr,                                     /*  Input line nbr from schema file */
...
static FILE *schema,                                       /*  Input file stream */
*output_stream = NULL;                                     /*  Generated code output file */

static ROWT *row_overflow;                                 /*  First overflow row, if any */

static sym_tab *var_tab,                                   /*  User-defined variables go here */
*int_tab;                                                  /*  Internal module names */

static symbol *std_array_base,                             /*  Base for generated tables */
...
static void init_standard_values(
    void
);
static void refresh_symbol_pointers(
    void
);
static void init_do_block_values(
    DO_BLOCK * do_block
);
static void init_do_block_symbols(
    void
);
static void init_if_block_values(
    IF_BLOCK * do_block
);
...
static event_t parse_schema_line(
    void
);
static void preformat_line_if_reqd(
    char *curline
);
...
static void get_name_token(
    int case_convert
);
static void get_string_token(
    void
);
static void get_filename_token(
    void
);
static void skip_spaces(
    void
);
static void expand_symbols(
    char *line
);
static void recalc_standard_values(
    void
);
static int extract_token(
    char *line,
    int case_convert
);
static void fix_quoted_strings(
    char *line,
    char from,
    char to
);
static int lookup_do_handler(
    void
);
static void insert_string_symbol(
    char *line,
    char *symbol
);
static void insert_numeric_symbol(
    char *line,
    long int symbol
);
static void insert_variable_value(
    char *line,
    char *name
);
static void get_action_filename(
    char *action
);
static int eval_condition(
    char *line
);
static void store_var_value(
    symbol * var_sym,
    char *var_value
);
static void check_option_value(
    symbol * var_sym
);
static void check_name_length(
    char *name,
    char *type
);
static FILE *open_extend(
    char *filename
);
static void write_output_line(
    char *line
);
static void clear_row_parts(
    void
);
static char *row_format(
    int row_index
);
static int row_field(
    int row_value
);
static ROWT *cat_token_to_row(
    ROWT * row,
    char *token
);
static void build_stub_header(
    char *name
);
static void prtitle(
    int width
...
)
{
    ...
    } else {                                               /*  Initialise global variables: */
...
)
{
    ...
    actions = (dbyte *) Check(malloc(sizeof(dbyte) * (stats->maxaction + 1)));
    vectors = (dbyte *) Check(malloc(LR_VECTORMAX * sizeof(dbyte) * (stats->maxaction + 1)));
...
)
{
    ...
    static ACTION_LOOKUP lookup[] = {                      /*  Table of schema :actions */
        { "close", close_event },
        { "copy", copy_event },
    ...
    ACTION_LOOKUP *lptr;                                   /*  Index into lookup table */

    event_t line_event;                                    /*  Returned event for line */
...
)
{
    if (schema_format == LANG_NONE) {                      /*  Check first time only */
        if (strlen(line) == COBOL_WIDTH && isdigit(line[0])
            && isdigit(line[1])
            && isdigit(line[2])
            && isdigit(line[3])
            && isdigit(line[4])
    ...
    if (schema_format == LANG_COBOL) {
        ...
        memmove(line, line + COBOL_MARGIN_A, strlen(line + COBOL_MARGIN_A) + 1);
    }
}
...
)
{
    ...
    for (scan = line; *scan; scan++) {
        ...
        } else if (*scan == std_comment_ind->svalue[0] && !in_string && strstr(scan, std_comment_ind->svalue) == scan) {
...
)
{
    ...
    if (isdigit(thisch)
        && (type == WANT_ANY)) {
        /*  Collect number token */
        get_number_token();
    } else if (isalpha(thisch) && (type == WANT_ANY || type == WANT_NAME)) {
        /*  Collect name token */
        get_name_token(convert);
    } else if (is_quote(thisch) && (type == WANT_ANY || type == WANT_STRING || type == WANT_FILENAME)) {
        /*  Collect string token */
        get_string_token();
    } else if (is_fname(thisch) && (type == WANT_FILENAME)) {
        /*  Collect filename token */
        get_filename_token();
    } else if (thisch == '$' && (type == WANT_NAME)) {
        /*  Collect $xxxx token */
        get_name_token(convert);
    } else {
        /*  If nothing, return empty token */
        *token = 0;
    }
...
MODULE declare_user_variable(
    void
)
{
    symbol *var_sym;                                       /*  Pointer into symbol table */
    ...
    if (streq(token, "int")) {                             /*  Get variable name */
        get_next_token(WANT_NAME, CV_LC);
        type = SYM_TYPE_INT;
    } else if (streq(token, "string")) {                   /*  Get variable name */
        get_next_token(WANT_NAME, CV_LC);
    ...
    if (*token == 0) {                                     /*  If no name, give-up */
        ...
        } else if (streq(token, "comma") || streq(token, "name") || streq(token, "number") || streq(token, "offset")
                   || streq(token, "tally") || streq(token, "row")
            ) {
            /*  Error if we have one of the standard :do block  variables */
...
)
{
    ...
    if (is_quote(*line)) {                                 /*  If quoted string, */
        *token = *line;                                    /*    start token with delimiter */
        ...
        while (*line != *token) {                          /*  Skip to closing quote */
            ...
            if (*line == '\\') {                           /*  Handle escaped character */
                StrClose(line, TRUE);
                if (*line == 'n') {
                    *line = '\n';                          /*  Newline */
                } else if (*line == 't') {
        ...
        /*  If plain token: copy characters */
        if (isupper(*line)) {
        ...
        while (isalpha(*line) || isdigit(*line) || *line == '_') {
            ...
            token[size++] = (char) (convert == CV_LC ? tolower(*line) : convert == CV_UC ? toupper(*line) : (*line));
            StrClose(line, TRUE);
        }
...
)
{
    ...
    for (scan = line; *scan; scan++) {
        ...
        } else if (is_quote(*scan)) {
            in_string = 1 - in_string;                     /*  Toggle string in-out switch */
        } else if (*scan == from && in_string) {
...
)
{
    ...
    if ((var_sym->type == SYM_TYPE_INT) || (var_sym->type == SYM_TYPE_EITHER && var_sym->svalue == NULL)
        ) {
...
{
    int calc_feedback,                                     /*  Feedback from lr_calculate () */
    ...
    if ((var_sym->type == SYM_TYPE_STRING)
        || (var_sym->type == SYM_TYPE_EITHER && calc_feedback != 0)) {  /*  Parse (string) token fom value */

        memmove(curline, value, strlen(value) + 1);
        char_nbr = 0;
        get_next_token(WANT_ANY, CV_NO);
        sym_values(var_sym, 0, token);
    } else if (var_sym->type == SYM_TYPE_INT) {
...
{
    static Bool options_checked = FALSE;                   /*  Check options first time */
...
)
{
    ...
    *include_to;                                           /*  End at this line (optional) */
    ...
    FILE *include_file;                                    /*  File we read from */
    Bool include_this;                                     /*  If TRUE, want current line */
    ...
    if (include_file == NULL) {
        ...
        if (output_stream == NULL) {
            ...
            while (FileRead(include_file, curline)) {
                ...
                if (!include_this && streq(curline, include_from) && *include_from) {
                    include_this = TRUE;
                } else if (include_this && streq(curline, include_to) && *include_to) {
                    include_this = FALSE;
                } else if (include_this) {
...
{
    static char output_line[LINE_MAX + 1];
...
)
{
    ...
    *source_name = NULL,                                   /*  Start at this line (optional) */
    ...
    if (!exception_raised) {                               /*  Get target name if no errors */
...
)
{
    ...
    *old_name = NULL,                                      /*  Start at this line (optional) */
    ...
    if (!exception_raised) {                               /*  Get new name if no errors */
...
MODULE output_line_of_source(
    void
)
{
    static dbyte last_error_line = 0;                      /*  Filter serial messages */
...
MODULE write_message_to_stdout(
    void
)
{
...
MODULE parse_internal_module_name(
    void
)
{
    ASSERT(int_tab != NULL);
...
MODULE push_variable_value(
    void
)
{
    symbol *old_sym,                                       /*  Old symbol */
...
MODULE parse_substring_specification(
    void
)
{
    ...
    if (substr_from > LINE_MAX || substr_from + substr_size > LINE_MAX) {
...
)
{
    ...
    if (!exception_raised) {                               /*  If we found :enddo, */
        get_next_token(WANT_NAME, CV_LC);
        if ((*token) && (do_block.handler != do_handler[lookup_do_handler()].handler)) {
            /*  If anything follows, check */
...
MODULE open_do_block_level(
    void
)
{
    DO_BLOCK *parent;

    if (do_block_level) {                                  /*  If do_block is active, save as parent of new do_block */
...
MODULE close_do_block_level(
    void
)
{
    DO_BLOCK *parent;

    if (do_block_level > 1) {                              /*  If 2 or greater, we have a parent do_block on the stack */
...
MODULE check_do_is_not_reentrant(
    void
)
{
    DO_BLOCK *parent;
...
static void handler_for_do_state(
    void
)
{
    dbyte state_nbr;
    ...
    if (state_nbr < stats->states) {
        strcpy(do_block.name_symbol, TrueName(stats->snames[state_nbr]));
        strcpy(do_block.comma_symbol, (state_nbr < stats->states - 1 ? std_comma_before->svalue : std_comma_last->svalue));
        the_next_event = ok_event;
    }
}
...
static void handler_for_do_event(
    void
)
{
    static Bool local_event_list;                          /*  Do we want a local event list? */

    dbyte event_nbr = 0;

    if (do_block.iteration == 0) {                         /*  First time - parse command  Look for 'local' keyword */
    ...
    if (local_event_list) {
        if (do_loop_event) {
            ...
            the_next_event = ok_event;
            sym_values(std_next_state, 0, TrueName(stats->snames[do_loop_event->child->number]));
    ...
    if (the_next_event == ok_event) {
        ...
        strcpy(do_block.comma_symbol, (event_nbr < stats->events - 1 ? std_comma_before->svalue : std_comma_last->svalue));
...
static void handler_for_do_module(
    void
)
{
    static Bool local_module_list;                         /*  Do we want a local module list? */
    ...
    if (the_next_event == ok_event) {
        ...
        strcpy(do_block.comma_symbol, (module_nbr < stats->modules - 1 ? std_comma_before->svalue : std_comma_last->svalue));
...
static void handler_for_do_action(
    void
)
{
    ...
    *module;
    ...
    if (state) {                                           /*  List ends when state is null */
        ...
        for (eventnbr = 0; eventnbr < stats->events; eventnbr++) {
            ...
            sprintf(token, row_format(eventnbr), (long) row_field(transit[eventnbr]));
            row = cat_token_to_row(row, token);
        }

        strcpy(do_block.comma_symbol, (state->next ? std_comma_before->svalue : std_comma_last->svalue));
        state = state->next;
        the_next_event = ok_event;
    }
}
...
{
    if ((dbyte) (strlen(row->symbol) + strlen(token)) > (dbyte) (std_row_width->ivalue)) {
...
static void handler_for_do_nextst(
    void
)
{
    static lrnode *state;                                  /*  Pointer to state in dialog list */
...
static void handler_for_do_vector(
    void
)
{
    static int offset;                                     /*  Offset of vector in output table */
    ...
    if (vecnbr < vectorcnt) {
        ...
        for (modnbr = 0;; modnbr++) {
            ...
            sprintf(token, row_format(modnbr), (long) nextvec[modnbr] + array_base);
            row = cat_token_to_row(row, token);
        }

        nextvec += stats->maxaction + 1;
        strcpy(do_block.comma_symbol, (vecnbr < vectorcnt - 1 ? std_comma_before->svalue : std_comma_last->svalue));
        the_next_event = ok_event;
    }
}
...
static void handler_for_do_overflow(
    void
)
{
    ...
    if (row) {                                             /*  If more overflow, get values */
        ...
        row = row->next;
        the_next_event = ok_event;
        strcpy(do_block.comma_symbol, (row ? std_comma_before->svalue : std_comma_last->svalue));
    }
}
...
static void handler_for_do_stubs(
    void
)
{
    static char flags[LR_MODULEMAX],                       /*  Indicate whether modules used */
     stubs[LR_FILENAMEMAX + 1];                            /*  Actual name of stubs file */
    static int newmods,                                    /*  Number of new modules required */
     modnbr;                                               /*  Used to scan through modules */
    static symbol *intsym;                                 /*  Internal module name symbol */

    char
    *name_ptr,                                             /*  Start of name in scanned line */
    *suffix_ptr,                                           /*  Start of suffix in module line */
    *prefix_ptr;                                           /*  Start of prefix in module line */
    int name_size,                                         /*  Size of name, if found */
     prefix_size,                                          /*  Size of prefix in module line */
     suffix_size;                                          /*  Size of suffix in module line */

    FILE *input;                                           /*  Files that contain modules */
    Bool module_done;                                      /*  Did we output something? */
        ...
        while (*token != 0) {
            ...
            while (FileRead(input, srcline)) {
                ...
                if (strncmp(prefix_ptr, srcline, prefix_size) == 0
                    ...
                    && (strchr(name_ptr, ' ') == NULL || strchr(name_ptr, ' ') >= (name_ptr + name_size))) {
                    ...
                    for (modnbr = 0; modnbr < stats->modules; modnbr++) {
                        if (streq(token, TrueName(stats->mnames[modnbr])) && flags[modnbr] == 0) {
                            flags[modnbr] = 1;
                    ...
                    if (modnbr >= stats->modules) {
                        for (intsym = int_tab->syms; intsym; intsym = intsym->next) {
...
)
{
    ...
    if (*std_stub_between->svalue) {                       /*  If stub_between is not empty center module name */
        repcount = (int) std_stub_width->ivalue - strlen(std_stub_first->svalue) - strlen(std_stub_last->svalue);
...
static void handler_for_do_while(
    void
)
{
...
{
    int eval_feedback,                                     /*  Feedback from lr_evaluate () */
     eval_offset,                                          /*  Position of any error */
     indent;                                               /*  For nice error messages */

    Bool eval_result = 0;                                  /*  Result of comparison */
...
MODULE open_if_block_level(
    void
)
{
    IF_BLOCK *parent;

    if (if_block_level) {                                  /*  If if_block is active, save as parent of new if_block */
...
MODULE close_if_block_level(
    void
)
{
    IF_BLOCK *parent;

    if (if_block_level > 1) {                              /*  If 2 or greater, we have a parent if_block on the stack */
...
MODULE trace_symbols_if_wanted(
    void
)
{
...
{
    Trace("   %-20s  %s  %4ld  \"%s\"", sp->name, sp->type == SYM_TYPE_INT ? "int" : sp->type == SYM_TYPE_STRING ? "str" :
          /*  else  */ "any",
          sp->ivalue, sp->svalue ? sp->svalue : "");
    return (1);
}
