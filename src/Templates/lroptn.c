/*===========================================================================*
 *                                                                           *
 *  lroptn.c    parse option definition, and other option functions          *
 *                                                                           *
...
static option option_save[OPT_MAX];
...
)
{
    ...
    *first;                                                /*  First char on line */
    ...
    options_file = FileWhere('r', OPT_DPATH.value ? OPT_DPATH.value : OPT_PATH.value, filename, NULL);

    if (options_file == NULL) {
        if (OPT_INITIAL.flags & OPT_ON) {
            fprintf(stderr, "Options file '%s' not found\n", filename);
        }
...
void lr_parse_option_line(
    char *options_line
)
{
    static char line[LINE_MAX + 1];                        /*  Copy of options line */
    ...
    *next;
    ...
    for (token = line; *token;) {
        ...
        if (*token == '!' || *token == 0) {
            /*  Comment - ignore rest of line ; End of line - quit */
            break;
        }

        if (*token != '-') {
            fprintf(stderr, "Bad option name '%s'\n", token);
            break;
        }
...
)
{
    ...
    *scan;                                                 /*  Scan and parse value string */
    ...
    length = strlen(name);
    if (length < 3) {
        fprintf(stderr, "Option name '%s' is too short\n", name);
        return (-1);
    }
    ...
    if (negate) {
        if (optptr->type == '?') {
            ...
            fprintf(stderr, "'-no%s' is meaningless\n", name);
        }
    } else {
        optptr->flags = OPT_ON;
    }

    if (optptr->type == '=' || optptr->type == '+') {      /*  New value for option   Added value for option */
        if (*value == 0) {
            ...
            if ((*value == '\'' || *value == '"') && (strlast(value) == *value)) {
        ...
        if (optptr->type == '=' || optptr->value == NULL) {
            if (optptr->value) {                           /*  Deallocate value if any */
                free(optptr->value);
            }
            optptr->value = StrDup(value);
        } else {                                           /*  '+' = Append this value  with ',' delimiter */
    ...
    } else if (*value) {                                   /*  ? options can only take  yes/no/true/false values */
        StrLwr(value);                                     /*  Get name into lower-case */

        if (streq(value, "yes") || streq(value, "true")) {
            optptr->flags = OPT_ON;
        } else if (streq(value, "no") || streq(value, "false")) {
            optptr->flags = OPT_OFF;
        } else {
            fprintf(stderr, "Unknown value for '-%s' ignored\n", name);
        }
    }
    return (0);
}

void lr_reset_options(
    void
)
{
...
void lr_push_options(
    void
)
{
    int optnbr;

    memcpy((void *) option_save, (void *) option_list, sizeof(option) * OPT_MAX);
...
)
{
    ...
    memcpy((void *) option_list, (void *) option_save, sizeof(option) * OPT_MAX);
}

void lr_show_options(
    char *filename
)
{
    int optnbr;                                            /*  Option number in list */
