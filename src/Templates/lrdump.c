...
)
{
    lrnode *state,
    *event,
    *module;                                               /*  Pointers into dialog list        */
    int item;                                              /*  Number of item to display        */
    char *typeptr,
    *nameptr;                                              /*  Pointers into symbol table       */
    byte *numbptr;
    ...
    for (typeptr = listhead->name; *typeptr;) {
        ...
        Trace("%c %3d %s", *typeptr, (*numbptr << 8) + *(numbptr + 1), nameptr);
    ...
    for (state = listhead->child; state; state = state->next) {
        ...
        for (event = state->child; event; event = event->next) {
            module = event->child;
            Trace("    (%02d) %-40s -> %3d %s", event->number, event->name, module->number, module->name);
